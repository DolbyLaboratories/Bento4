...
|   constants
+---------------------------------------------------------------------*/
const AP4_UI32 AP4_LABL_FLAG_IS_GROUP_LABEL_MASK = 0x00000001;
...
AP4_LablAtom::AP4_LablAtom(AP4_Flags   is_group_label,
                           ...
                           const char* language,
                           const char* label) :
    AP4_Atom(AP4_ATOM_TYPE_LABL, AP4_FULL_ATOM_HEADER_SIZE+2, 0, is_group_label ? AP4_LABL_FLAG_IS_GROUP_LABEL_MASK : 0),
        m_LabelID(label_id)
{
    if (language) {
        m_Language = language;
...
/*----------------------------------------------------------------------
|   AP4_LablAtom::GetIsGroupLabel
+---------------------------------------------------------------------*/
AP4_Flags AP4_LablAtom::GetIsGroupLabel()
{
    return (m_Flags & AP4_LABL_FLAG_IS_GROUP_LABEL_MASK) ? true : false;
}

/*----------------------------------------------------------------------
|   AP4_LablAtom::SetLanguage
+---------------------------------------------------------------------*/
void AP4_LablAtom::SetIsGroupLabel(AP4_Flags is_group_label)
{
    if (is_group_label) {
        m_Flags |= AP4_LABL_FLAG_IS_GROUP_LABEL_MASK;
    } else {
        m_Flags &= ~AP4_LABL_FLAG_IS_GROUP_LABEL_MASK;
    }
}
...
{
    inspector.AddField("is_group_label", (m_Flags & AP4_LABL_FLAG_IS_GROUP_LABEL_MASK)  ? true : false);
