...
public:
    ...
    struct InputConfig {
        AP4_List<ConfigSection> sections;
    };

    AP4_Preselection (std::unique_ptr<InputConfig> input_config);
...
private:
    static std::unique_ptr<AP4_Preselection::InputConfig> parseConfigFileToInput(const char* config_file);
    static AP4_Result parsePreselectionSection(ConfigSection *current_section, const char *key, const char *value);
    void normalizeConfigSections();
