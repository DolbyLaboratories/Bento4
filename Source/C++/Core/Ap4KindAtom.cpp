    ...
    AP4_Atom(AP4_ATOM_TYPE_KIND, size, version, flags)
{
    ...
    if (size > AP4_FULL_ATOM_HEADER_SIZE + m_SchemeURI.GetLength() + 1) stream.ReadNullTerminatedString(m_Value);
}
...
AP4_KindAtom::WriteFields(AP4_ByteStream& stream)
{
    ...
    if (m_Value.GetLength() > 0) result = stream.WriteNullTerminatedString(m_Value.GetChars());
    if (AP4_FAILED(result)) return result;
...
AP4_KindAtom::InspectFields(AP4_AtomInspector& inspector)
{
    ...
    if (m_Value != "") inspector.AddField("value", m_Value.GetChars());
