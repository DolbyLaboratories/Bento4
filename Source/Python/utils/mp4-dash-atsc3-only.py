...
from mp4utils import MakePsshBox,\
                     ...
                     DolbyVisionDualEntry,\
                     ContainDolbyVision,\
                     ReGroupVideoSetsDASH,\
                     ReOrderMediaTrack,\
                     ReGroupAC4andEC3Sets,\
                     ...
                     ReGroupVideoSetsHLS,\
                     FindAudioGroups,\
                     NeedReGroupDlbAudioSets,\
                     FindDependencyId, \
                     BENTO4_UTILS_VERSION
...
TRIVENI_SEGMENT_URL_TEMPLATE= '$RepresentationID$_seg-$Number$.m4s'
...
DASHIF_NAMESPACE            = 'https://dashif.org/CPS'
...
ISOFF_BROADCAST_PROFILE     = 'urn:mpeg:dash:profile:isoff-broadcast:2015'
HBBTV_15_ISOFF_LIVE_PROFILE = 'urn:hbbtv:dash:profile:isoff-live:2012'

DVB_DASH_2014_ISOFF_LIVE_PROFILE = 'urn:dvb:dash:profile:dvb-dash:2014'
DVB_DASH_2017_ISOFF_LIVE_PROFILE = 'urn:dvb:dash:profile:dvb-dash:2017'

ProfileAliases = {
  'main'        : ISOFF_MAIN_PROFILE,
  'live'        : ISOFF_LIVE_PROFILE,
  'on-demand'   : ISOFF_ON_DEMAND_PROFILE,
  'broadcast'   : ISOFF_BROADCAST_PROFILE,
  'hbbtv'       : HBBTV_15_ISOFF_LIVE_PROFILE
}

XMLNS_XSI  = 'http://www.w3.org/2001/XMLSchema-instance'
XMLNS_DLB  = 'http://www.dolby.com/ns/online/DASH'
XSI_SCHEMA = 'urn:mpeg:dash:schema:mpd:2011 DASH-MPD.xsd'
...
#############################################
def IndentXmlElement(element, level=0, indent='  '):
    children = list(element)
    if children:
        if not element.text or not element.text.strip():
            element.text = '\n' + indent * (level + 1)
        for child in children:
            IndentXmlElement(child, level + 1, indent)
...
#############################################
def XmlToPrettyString(element):
    element_copy = copy.deepcopy(element)
    IndentXmlElement(element_copy)
    return xml.tostring(element_copy, encoding='unicode')
...
def AddSegmentTemplate(options, container, init_segment_url, media_url_template_prefix, track, stream_name):
    ...
    if options.use_segment_timeline or track.type == 'subtitles':
        if options.triveni == True:
            url_template = TRIVENI_SEGMENT_URL_TEMPLATE
        else:
        ...
        if options.triveni == True:
            final_media_name = TRIVENI_SEGMENT_URL_TEMPLATE
        else:
            final_media_name = SEGMENT_URL_TEMPLATE
        xml.SubElement(container,
                       'SegmentTemplate',
                       timescale='1000',
                       duration=str(int(round(track.average_segment_duration*1000))),
                       initialization=init_segment_url,
                       media=final_media_name,
                       startNumber='1') # (keep the @startNumber, even if not needed, because some clients like Silverlight want it)
...
def AddContentProtection(options, container, tracks, all_tracks):
    ...
    if options.playready:
        ...
        pssh = xml.SubElement(cp, '{' + CENC_2013_NAMESPACE + '}pssh')
        pssh.text = pssh_b64
        if options.playready_license_uri:
            # DASH-IF-IOP-Part6-v5.0.0 7.3
            old_laurl = xml.SubElement(cp, '{' + DASHIF_NAMESPACE + '}Laurl')
            old_laurl.text = options.playready_license_uri
            # DASH-IF implementation guidelines: content protection and security (24 October 2024) 9.3
            # laurl = xml.SubElement(cp, '{' + DASHIF_NAMESPACE + '}laurl')
            # laurl.text = options.playready_license_uri
    ...
    if options.widevine:
        ...
        if options.widevine_header:
            ...
            pssh = xml.SubElement(cp, '{' + CENC_2013_NAMESPACE + '}pssh')
            pssh.text = pssh_b64
            if options.widevine_license_uri:
                old_laurl = xml.SubElement(cp, '{' + DASHIF_NAMESPACE + '}Laurl')
                old_laurl.text = options.widevine_license_uri
                # laurl = xml.SubElement(cp, '{' + DASHIF_NAMESPACE + '}laurl')
                # laurl.text = options.widevine_license_uri
...
#############################################
def AddDescriptor(adaptation_set_or_preselection, set_attributes, set_name, category_name, element_name, options):
    label_sets = {}
    descriptor_list  = []
    ...
    if not attributes and element_name == 'Preselection': 
        return label_sets, descriptor_list
    elif not attributes and element_name == 'AdaptationSet':
        return 
    
    if ISOFF_BROADCAST_PROFILE in options.profiles or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
        element_role = ''
        if element_name == 'AdaptationSet' or element_name == 'Preselection':
            if 'Role' in attributes:
                element_role = attributes['Role']
            # support using lowercase names instead of capitalized names
            elif 'role' in attributes:
                element_role = attributes['role']
            # Manifest (MPD) files: Required Manual changes - #12
            if element_name == 'Preselection' and element_role == 'alternate':
                attributes['{urn:mpeg:dash:role:2011}Accessibility'] = 'description'

    descriptor_names = ['Accessibility', 'Role', 'Rating', 'Viewpoint', 'SupplementalProperty', 'EssentialProperty', 'Label']
    for descriptor_name in attributes:
        ...
        for descriptor_value in descriptor_values.split(','):
            ...
            if descriptor_name == 'supplementalproperty': descriptor_name = 'SupplementalProperty'
            if descriptor_name == 'essentialproperty'   : descriptor_name = 'EssentialProperty'
            if descriptor_name == 'label'               : descriptor_name = 'Label'
            
            if ISOFF_BROADCAST_PROFILE in options.profiles or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
                # Manifest (MPD) files: Required Manual changes - #5
                if element_name == 'AdaptationSet' and (element_role == 'main' or element_role == 'dub') and descriptor_name == 'Accessibility':
                    continue
                
                # Manifest (MPD) files: Required Manual changes - #6
                if element_name == 'AdaptationSet' and descriptor_name == 'Accessibility' and descriptor_namespace == 'urn:mpeg:dash:role:2011':
                    continue
            
            if descriptor_name not in descriptor_names:
                continue
            if descriptor_name == 'Label':
                if not descriptor_namespace:
                    descriptor_namespace = 'en'
                label_sets[descriptor_namespace] = descriptor_value
            else:
                if descriptor_namespace and descriptor_name != 'Label':
...
def formatDASHManifest(mpd, mpd_ns):
    indent_str = '  '
    mpd_lines = mpd.split('\n')
    for line in mpd_lines:
        if 'xml version="1.0"' in line:
            mpd_lines[mpd_lines.index(line)]  = line.replace('xml version="1.0" ', 'xml version="1.0" encoding="utf-8"')
        if '<MPD' in line:
            manifest_attr_line = line
        # tags issue, TODO: figure out the reason, and resolve it
        if '<Preselection' in line:
            mpd_lines[mpd_lines.index(line)]  = line.replace('tags', 'tag')
    attributes = manifest_attr_line.split(' ')
    attributes_list = []
    for attr in attributes:
        if attr == '<MPD':
            ...
            continue
        # two space indent
        attr = indent_str + attr 
        if attr.find(indent_str + 'xmlns="' +  mpd_ns + '"') != -1:
           attr = attr.replace(indent_str + 'xmlns="'              +  mpd_ns + '"',
                               indent_str + 'xmlns:xsi="'          +  XMLNS_XSI  + '"' + '\n' +
                               indent_str + 'xmlns:dolby="'        +  XMLNS_DLB  + '"' + '\n' + 
                               indent_str + 'xmlns="'              +  mpd_ns     + '"' + '\n' +
                               indent_str + 'xsi:schemaLocation="' +  XSI_SCHEMA + '"' )
        attributes_list.append(attr)
    mpd_lines[mpd_lines.index(manifest_attr_line)] = '\n'.join(attributes_list)
    return '\n'.join(mpd_lines)

# multi-PID for AC-4
def multiPreselectionComponents(audio_tracks, item, presentation_id):
    preselection_components = []
    cur_idx = audio_tracks.index(item)
    for idx in range(len(audio_tracks)):
        tracks = audio_tracks[idx]
        if cur_idx == idx or tracks[0].codec_family != 'ac-4':
            continue
        for track in tracks:
            presentations = track.info['sample_descriptions'][0]['dolby_ac4_info']['presentations']
            for pres in presentations:
                if pres['presentation_id'] == presentation_id:
                    preselection_components.append(track.adaptationset_id)
    return preselection_components
...
def OutputDash(options, set_attributes, audio_sets, video_sets, subtitles_sets, subtitles_files):
    ...
    adaptation_set_id  = 10
    preselection_id    = 1000
    label_id           = 10000
    pres_label_text_id = 0
    ...
    if ISOFF_BROADCAST_PROFILE in options.profiles or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
        ...
        mpd = xml.Element('MPD',
                          ...
                          minimumUpdatePeriod = 'PT0.00S',
                          publishTime = '%d-%02d-%02dT%02d:%02d:%02dZ' % (tm.tm_year, tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec),
                          availabilityStartTime = '%d-%02d-%02dT%02d:%02d:%02dZ' % (tm.tm_year, tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec),
                          type = 'dynamic')
    else:
        mpd = xml.Element('MPD',
                          xmlns=mpd_ns,
                          ...
                          mediaPresentationDuration = XmlDuration(presentation_duration),
                          type = 'static')
    mpd.append(xml.Comment(' Created with Bento4 mp4-dash-atsc3-only.py, VERSION=' + VERSION + '-' + SDK_REVISION + ' '))
    period_extras = {'id': '1', 'start': 'PT0S'}
    period = xml.SubElement(mpd, 'Period', **period_extras)
    ...
    if video_sets:
        ...
        if HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles and len(ordered_video_track) > 1:
            print('Please set at least one AdaptationSet with a Role element according to ISO/IEC 23009-1 clause 8.8 and 8.9.')
        for video_tracks in ordered_video_track:
            ...
            adaptation_set = xml.SubElement(period,
                                            'AdaptationSet',
                                            id = str(adaptation_set_id),        #TODO 2021 this is necessary
            ...
            AddDescriptor(adaptation_set, set_attributes, 'video/' + str(adaptation_set_id), 'video', 'AdaptationSet', options)

            if hasattr(video_tracks[0], 'supplemental_codec') and hasattr(video_tracks[0], 'supplemental_profile'):
                print('Using new SCTE signaling scheme for Dolby Vision Profile 8.1/8.4 ...')
                property_type_string = 'EssentialProperty'
                if options.dv_use_supplemental == True:
                    property_type_string = 'SupplementalProperty'
                #adding MatrixCoefficients, except for SDR content
                if video_tracks[0].matrix_coefficients != '1':
                    xml.SubElement(adaptation_set,
                                   property_type_string,
                                   schemeIdUri=MPEG_DASH_MATRIX_COEFFICIENTS_SCHEME_ID_URI,
                                   value=video_tracks[0].matrix_coefficients)

                #adding ColourPrimaries, except for SDR content
                if video_tracks[0].colour_primaries != '1':
                    xml.SubElement(adaptation_set,
                                   property_type_string,
                                   schemeIdUri=MPEG_DASH_COLOUR_PRIMARIES_SCHEME_ID_URI,
                                   value=video_tracks[0].colour_primaries)

                #adding TransferCharacteristics, except for SDR content
                if video_tracks[0].colour_primaries != '1':
                    xml.SubElement(adaptation_set,
                                   property_type_string,
                                   schemeIdUri=MPEG_DASH_TRANSFER_CHARACTERISTICS_SCHEME_ID_URI,
                                   value=video_tracks[0].transfer_characteristics)
            ...
            if options.on_demand:
                ...
                # media_segment_url_template_prefix is not used now.
                if options.split:
                    if options.triveni == True:                                 # TODO 2021 do not create sub-folder
                        init_segment_url                  = '$RepresentationID$_' + SPLIT_INIT_SEGMENT_NAME
                        media_segment_url_template_prefix = '$RepresentationID$_'
                    else:
            ...
            for video_track in video_tracks:
                if round(float(video_track.frame_rate),2) > 29.97:
                    options.have_HFR = True
                if hasattr(video_track, 'supplemental_codec') and hasattr(video_track, 'supplemental_profile'):
                    ...
                    representation.set('{'+SCTE_NAMESPACE+'}supplementalCodecs', video_track.codec.split(',')[1])
                    representation.set('{'+SCTE_NAMESPACE+'}supplementalProfiles', video_track.supplemental_profile)

                    if options.segment_profiles:
                        representation.set('segmentProfiles', video_track.segment_profile)
                else:
                    representation = xml.SubElement(adaptation_set,
                                                    'Representation',
                                                    id=video_track.representation_id,
            ...
            # increase the AdaptationSet ID
            adaptation_set_id += 1

    # process the audio tracks
    if audio_sets:
        period.append(xml.Comment(' Audio '))
        # Re-group audio sets for AC-4 and EC3 stream. (add channel attributes, and whether it's self contained (Mulit-PID))
        # fat stream and slim stream should not be in the same Adapatation Set.
        # DDP and DDP JOC should not be in the same Adaptation Set
        audio_sets = ReGroupAC4andEC3Sets(audio_sets)
 #       audio_sets = ReGroupEC3Sets(audio_sets)
        if NeedReGroupDlbAudioSets(audio_sets):       #TODO 2021 find if it's necessary to do this.
            audio_sets = ReGroupDlbAudioSets(audio_sets)
        ...
        for audio_tracks in ordered_audio_track:
            ...
            # codecs is required for both ac4 and ec3
            kwargs = {'id': str(adaptation_set_id), 'mimeType': AUDIO_MIMETYPE, 'startWithSAP': '1', 'segmentAlignment': 'true', 'codecs': audio_tracks[0].codec, 'contentType': 'audio'}
            language = audio_tracks[0].language
#            if language == 'und' and audio_tracks[0].codec_family == 'ac-4':
#                presentataion_info = audio_tracks[0].info['sample_descriptions'][0]['dolby_ac4_info']['presentations']
#                language = presentataion_info[0]['Language'] if presentataion_info[0]['Language'] != 'un' else 'und'
...
#                kwargs['lang'] = language
            ...
            # Manifest (MPD) files: Required Manual changes - #4
            # TODO 2021 in broadcast profile, put AudioChannelConfiguration in AdaptationSet
            # AudioChannelConfiguration in AdaptationSet is only for AC4 stream (including slim stream, referring to specs)
            if ISOFF_BROADCAST_PROFILE in options.profiles or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
                if audio_tracks[0].codec_family == 'ac-4':
                    # audio_channel_config_value = ComputeDolbyAc4AudioChannelConfig(audio_tracks[0])
                    ...
                    for single in audio_tracks: 
                        if single.CM != 'CMNA':
                            set_attributes['audio/' + language] = {'Role':'main'}
                        if single.AA != 'AANA':
                            set_attributes['audio/' + language] = {'Role':'commentary', '{urn:tva:metadata:cs:AudioPurposeCS:2007}Accessibility':'1'}
            
            # see if we have descriptors
            AddDescriptor(adaptation_set, set_attributes, 'audio/' + str(adaptation_set_id), 'audio', 'AdaptationSet', options)
            ...
            for audio_track in audio_tracks:
                # Manifest (MPD) files: Required Manual changes - #1
                # process ec3/ec3 joc and ac3 stream
                
                # used for preselectionComponents
                audio_track.adaptationset_id = adaptation_set_id
                if audio_track.codec == 'ec-3' or audio_track.codec == 'ac-3':
                    options.have_Non_NGA = True
                    representation = xml.SubElement(adaptation_set,
                                                    'Representation',
                ...
                # process ac4 fat stream and slim stream
                elif audio_track.codec.startswith('ac-4'):
                    options.have_NGA = True
                    # if ISOFF_BROADCAST_PROFILE or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles and audio_track.dolby_ac4_ims == 'Yes':
                    #     PrintErrorAndExit('ERROR: AC-4 IMS are not recommend to be used for broadcast.\n')
                    
                    if audio_track.codec_family == 'ac-4' and audio_track.codec[0:7] != 'ac-4.02':
                        PrintErrorAndExit('ERROR: For AC-4, only support AC-4 BitStream version 2 (AC-4 specs part-2), please re-encode the AC-4 stream.\n')
                    
                    # process AC4 fat stream
                    presentations = audio_track.info['sample_descriptions'][0]['dolby_ac4_info']['presentations']
                    ac4_fat_stream = False
                    if len(presentations) > 1:
                        if presentations[0]['Codec String'][0:10] == 'ac-4.02.02' and len(presentations) == 2:
                            ac4_fat_stream = False
                        else:
                            ac4_fat_stream = True
                    else:
                    ...
                    if ac4_fat_stream == True:
                        if ISOFF_BROADCAST_PROFILE not in options.profiles and HBBTV_15_ISOFF_LIVE_PROFILE not in options.profiles:
                            PrintErrorAndExit('ERROR: Detected AC4 fat stream, please use --profiles broadcast or --profiles hbbtv.\n')
                        
                        #remove language attribute if an adaptation set is referred by more than one preselction
                        if 'lang' in adaptation_set.attrib:
                            adaptation_set.attrib.pop('lang')
                        # add representation for ac4 fat stream
                        representation = xml.SubElement(adaptation_set,
                                                        'Representation',
                                                        id=audio_track.representation_id,
                                                        bandwidth=str(audio_track.bandwidth))
                        # add preselection for AC4 fat stream
                        if audio_track.self_contained == 'No':
                            continue
                    else:
                        dependency_id = FindDependencyId(ordered_audio_track, audio_track.AA)
                        if dependency_id == -1:
                            representation = xml.SubElement(adaptation_set,
                                                            'Representation',
                                                            id=audio_track.representation_id,
                        ...
                        (mpeg_scheme, audio_channel_config_value) = DolbyAc4WithMPEGDASHScheme(audio_channel_config_value)
                        if (mpeg_scheme):
                            if options.dolby_channel_config == True and audio_channel_config_value == '0':
                                scheme_id_uri = DOLBY_AC4_AUDIO_CHANNEL_CONFIGURATION_SCHEME_ID_URI
                            else:
                                scheme_id_uri = MPEG_DASH_NEW_AUDIO_CHANNEL_CONFIGURATION_SCHEME_ID_URI
                        else:
                        ...
                        xml.SubElement(representation,
                                       ...
                                       schemeIdUri=scheme_id_uri,
                                       value=audio_channel_config_value)
                        if audio_track.dolby_ac4_ims == 'Yes':
                            xml.SubElement(representation,
                                           'SupplementalProperty',
                                           schemeIdUri='tag:dolby.com,2016:dash:virtualized_content:2016',
                                           value='1')
                            
                    #broadcast mode for fat stream will produce preselections
                    #for slim stream will not produce preselection
                    if ISOFF_BROADCAST_PROFILE in options.profiles or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
                        for pres in presentations:
                            # single IMS content does not need preselections
                            if pres['Codec String'][0:10] == 'ac-4.02.02' and len(presentations) == 2:
                                AddDescriptor(adaptation_set, set_attributes, 'audio/' + language, 'audio', 'AdaptationSet', options)
                                break
                            
                            if len(presentations) == 1:
                                break
                            
                            if pres['presentation_id'] == -1:
                                PrintErrorAndExit('ERROR: For %s or %s, each presentation must contain the presentation id\n' % ISOFF_BROADCAST_PROFILE, HBBTV_15_ISOFF_LIVE_PROFILE)
                                   
                            # find multi-PID for AC-4
                            add_preselection_components = []
                            preselection_components = str(audio_track.adaptationset_id)
                            # Not decodable presentation
                            if pres['Self Contained'] == 'No':
                                add_preselection_components = multiPreselectionComponents(ordered_audio_track, audio_tracks, pres['presentation_id']) # TODO 2021
                            for item in add_preselection_components:
                                preselection_components = preselection_components + ' ' + str(item)
                            
                            pre_args = [period, 'Preselection']
                            pre_kwargs = {'id' : str(preselection_id), 'preselectionComponents': preselection_components, 'tags': str(pres['presentation_id']), \
                                          'codecs': pres['Codec String'], 'audioSamplingRate': str(audio_track.sample_rate)}
                            language = LanguageCodeMap.get(pres['Language'], pres['Language'])
                            if language and language != 'un' and language != 'und':
                                pre_kwargs['lang'] = language
                            preselection = xml.SubElement(*pre_args, **pre_kwargs)
    
                            (mpeg_scheme, audio_channel_config_value) = DolbyAc4WithMPEGDASHScheme(('%06x' % pres['presentation_channel_mask_v1']).upper())
                            if (mpeg_scheme):
                                scheme_id_uri = MPEG_DASH_NEW_AUDIO_CHANNEL_CONFIGURATION_SCHEME_ID_URI
                            else:
                                scheme_id_uri = DOLBY_AC4_AUDIO_CHANNEL_CONFIGURATION_SCHEME_ID_URI
                            xml.SubElement(preselection,
                                           'AudioChannelConfiguration',
                                           schemeIdUri=scheme_id_uri,
                                           value=audio_channel_config_value) 
                            if pres['Codec String'][0:10] == 'ac-4.02.02' and len(presentations) > 2:
                                xml.SubElement(preselection,
                                                ...
                                                schemeIdUri='tag:dolby.com,2016:dash:virtualized_content:2016',
                                                value='1')
                            xml.SubElement(preselection, "SupplementalProperty", schemeIdUri="urn:mpeg:dash:preselection:2016", value='')
                            label_attrs, descriptor_list = AddDescriptor(preselection, set_attributes, 'preselection/' + str(pres['presentation_id']), 'preselection', 'Preselection', options)
                            
                            if label_attrs:
                                for name, value in label_attrs.items():
                                    Label = xml.SubElement(preselection, 'Label', id=str(label_id), lang=name)
                                    Label.text = value
                                    label_id += 1
                            else:
                                Label = xml.SubElement(preselection, 'Label', id=str(label_id))
                                Label.text = '(Pres. %s)' % pres_label_text_id
                                label_id += 1
                                pres_label_text_id +=1
    ...
    if subtitles_sets:
        ...
        for _, subtitles_tracks in list(subtitles_sets.items()):
            for subtitles_track in subtitles_tracks:
                ...
                AddDescriptor(adaptation_set, set_attributes, 'subtitles/' + subtitles_track.language, 'subtitles', None, options)
    ...
    if subtitles_files:
        ...
        for subtitles_file in subtitles_files:
            ...
            # see if we have other descriptors
            AddDescriptor(adaptation_set, set_attributes, 'subtitles/' + subtitles_file.language, 'subtitles', None, options)
    ...
    #update Manifest profiles for hbbtv profiles
    if HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
        if options.have_HDR or options.have_HFR:
            options.profiles.append(DVB_DASH_2017_ISOFF_LIVE_PROFILE)
        else:
            if options.have_NGA == True and options.have_Non_NGA == False:
                options.profiles.append(DVB_DASH_2017_ISOFF_LIVE_PROFILE)
            elif options.have_NGA == False and options.have_Non_NGA == True:
                options.profiles.append(DVB_DASH_2014_ISOFF_LIVE_PROFILE)
            elif options.have_NGA == True and options.have_Non_NGA == True:
                options.profiles.append(DVB_DASH_2014_ISOFF_LIVE_PROFILE)
                options.profiles.append(DVB_DASH_2017_ISOFF_LIVE_PROFILE)
            # no audio tracks
            elif options.have_NGA == False and options.have_Non_NGA == False:
                options.profiles.append(DVB_DASH_2014_ISOFF_LIVE_PROFILE)
        mpd.set('profiles', ','.join(options.profiles))
    
    # save the MPD
    if options.mpd_filename:
        mpd_xml = XmlToPrettyString(mpd)
        open(path.join(options.output_dir, options.mpd_filename), "w").write(formatDASHManifest(mpd_xml, mpd_ns))
...
#############################################
def OutputHlsCommon(options, track, all_tracks, media_subdir, playlist_name, media_file_name, hls_version=6):
    ...
    playlist_file.write('#\n')
    playlist_file.write('#EXT-X-VERSION:{}\n'.format(hls_version))
...
#############################################
def OutputHlsTrack(options, track, all_tracks, media_subdir, media_playlist_name, media_file_name, hls_version=6):
    media_playlist_file = OutputHlsCommon(options, track, all_tracks, media_subdir, media_playlist_name, media_file_name, hls_version)
...
#############################################
def OutputHlsWebvttPlaylist(options, media_subdir, media_playlist_name, media_file_name, total_duration, hls_version=6):
...
#############################################
def OutputHlsIframeIndex(options, track, all_tracks, media_subdir, iframes_playlist_name, media_file_name, hls_version=6):
    index_playlist_file = OutputHlsCommon(options, track, all_tracks, media_subdir, iframes_playlist_name, media_file_name, hls_version)
...
#############################################
def DetermineHlsVersion(all_video_tracks):
    for video_track in all_video_tracks:
        if (hasattr(video_track, 'info') and 
            'sample_descriptions' in video_track.info and 
            video_track.info['sample_descriptions']):
            
            sample_desc = video_track.info['sample_descriptions'][0]
            if 'dolby_vision' in sample_desc:
                dv_info = sample_desc['dolby_vision']
                if dv_info.get('profile') == 20:
                    return 12
...
def OutputHls(options, set_attributes, audio_sets, video_sets, subtitles_sets, subtitles_files):
    ...
    all_video_tracks     = sum(list(video_sets.values()),     [])
    all_subtitles_tracks = sum(list(subtitles_sets.values()), [])
    hls_version = DetermineHlsVersion(all_video_tracks)
    ...
    master_playlist_file.write('#EXT-X-VERSION:{}\n'.format(hls_version))
    master_playlist_file.write('\n')
    ...
    for audio_tracks in ordered_audio_track:
        ...
        if audio_channel_name.find('/JOC') != -1 or audio_channel_name.find('/IMSA') != -1:
        ...
        for audio_track in audio_tracks:
            ...
            OutputHlsTrack(options, audio_track, all_audio_tracks + all_video_tracks, media_subdir, media_playlist_name, media_file_name, hls_version)
        ...
        for idx in range(len(audio_groups)):
            ...
            for video_track in video_group:
                ...
                if audio_group_name != 'no_audio_group':
                    ...
                    # Check if this is Dolby Vision Profile 20 to add REQ-VIDEO-LAYOUT
                    req_video_layout = ''
                    if hasattr(video_track, 'info') and 'sample_descriptions' in video_track.info and video_track.info['sample_descriptions']:
                        sample_desc = video_track.info['sample_descriptions'][0]
                        if 'dolby_vision' in sample_desc:
                            dv_info = sample_desc['dolby_vision']
                            if dv_info.get('profile') == 20:
                                req_video_layout = 'REQ-VIDEO-LAYOUT="CH-STEREO",'
                    if supplemental_codec_string != '':
                        master_playlist_file.write('#EXT-X-STREAM-INF:{}{}AUDIO="{}",AVERAGE-BANDWIDTH={:.0f},BANDWIDTH={:.0f},VIDEO-RANGE={},CODECS="{}",SUPPLEMENTAL-CODECS="{}",RESOLUTION={:.0f}x{:.0f},FRAME-RATE={:.3f}\n'.format(
                                                   subtitles_group,
                                                   req_video_layout,
                                                   audio_group_name,
                                                   ...
                                                   video_track.dv_codec+','+audio_codec,
                                                   supplemental_codec_string,
                                                   video_track.width,
                                                   video_track.height,
                    ...
                    if hasattr(video_track, 'info') and 'sample_descriptions' in video_track.info and video_track.info['sample_descriptions']:
                        ...
                        if 'dolby_vision' in sample_desc:
                            ...
                            if dv_info.get('profile') == 20:
                                req_video_layout = 'REQ-VIDEO-LAYOUT="CH-STEREO",'
                    if supplemental_codec_string != '':
                        master_playlist_file.write('#EXT-X-STREAM-INF:{}{}AVERAGE-BANDWIDTH={:.0f},BANDWIDTH={:.0f},VIDEO-RANGE={},CODECS="{}",SUPPLEMENTAL-CODECS="{}",RESOLUTION={:.0f}x{:.0f},FRAME-RATE={:.3f}\n'.format(
                                               subtitles_group,
                                               req_video_layout,
                                               video_track.average_segment_bitrate,
                                               video_track.max_segment_bitrate,
                                               video_track.video_range,
                                               video_track.dv_codec,
                                               supplemental_codec_string,
                                               video_track.width,
                                               video_track.height,
                                               video_track.frame_rate))
                ...
                if processing_video_tracks:
                    ...
                    iframe_average_segment_bitrate,iframe_max_bitrate = OutputHlsIframeIndex(options, video_track, all_audio_tracks + all_video_tracks, media_subdir, iframes_playlist_name, media_file_name, hls_version)
                    ...
                    if hasattr(video_track, 'info') and 'sample_descriptions' in video_track.info and video_track.info['sample_descriptions']:
                        ...
                        if 'dolby_vision' in sample_desc:
                            ...
                            if dv_info.get('profile') == 20:
                                req_video_layout = 'REQ-VIDEO-LAYOUT="CH-STEREO",'
                    if supplemental_codec_string != '':
                        iframe_playlist_lines.append('#EXT-X-I-FRAME-STREAM-INF:{}AVERAGE-BANDWIDTH={:.0f},BANDWIDTH={:.0f},VIDEO-RANGE={},CODECS="{}",SUPPLEMENTAL-CODECS="{}",RESOLUTION={:.0f}x{:.0f},URI="{}"\n'.format(
                                                 req_video_layout,
    ...
    if subtitles_files:
        ...
        for subtitles_file in subtitles_files:
            ...
            master_playlist_file.write('#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subtitles",NAME="{}",DEFAULT={},AUTOSELECT=YES,LANGUAGE="{}",URI="{}/{}"\n'.format(
                                       ...
                                       media_playlist_name))
            select_as_default = False
            OutputHlsWebvttPlaylist(options, media_subdir, media_playlist_name, subtitles_file.media_name, presentation_duration, hls_version)
...
def OutputSmooth(options, audio_tracks, video_tracks):
    ...
    if options.smooth_client_manifest_filename != '':
        open(path.join(options.output_dir, options.smooth_client_manifest_filename), 'w').write(XmlToPrettyString(client_manifest))
    ...
    if options.smooth_server_manifest_filename != '':
        open(path.join(options.output_dir, options.smooth_server_manifest_filename), 'w').write(XmlToPrettyString(server_manifest))
...
def SelectTracks(options, media_sources):
    ...
    for media_source in media_sources:
        ...
        for track in tracks:
			...
			# in mp4mux channel count for ac-3 and ec-3 are set to be 2 by default.
        ...
        for track in [t for t in tracks if t.type == 'audio']:
            ...
            if not options.all_input_signals:
                # only keep this track if there isn't already a track with the same codec, language and channels at the same bitrate (within 10%)
                with_same_bandwidth = [t for t in adaptation_set if t.self_contained == 'Yes' and 
                                       abs(float(t.bandwidth-track.bandwidth)/float(t.bandwidth)) < 0.1 and 
                                       t.channels == track.channels and 
                                       track.CM == 'CMNA' and 
                                       track.AA == 'AANA']
        ...
        for track in [t for t in tracks if t.type == 'video']:
            if hasattr(track, 'dv_codec_family') == False:
                adaptation_set_name = ('video', track.codec_family)
            elif hasattr(track, 'supplemental_profile'):
                # for back compatiable profile
                adaptation_set_name = ('video', track.codec_family, track.dv_codec_family, track.supplemental_profile)
            else:
                # for non-back compatiable profile
                adaptation_set_name = ('video', track.codec_family, track.dv_codec_family)
            # adaptation_set_name = ('video', track.codec_family)
...
def main():
    ...
    parser.add_option('', '--profiles', dest='profiles',
                      help="Comma-separated list of one or more profile(s). Complete profile names can be used, or profile aliases ('live'='"+ISOFF_LIVE_PROFILE+"', 'on-demand'='"+ISOFF_ON_DEMAND_PROFILE+"', broadcast'='"+ISOFF_BROADCAST_PROFILE+"', 'hbbtv='"+HBBTV_15_ISOFF_LIVE_PROFILE+"')", default='live',
                      metavar="<profiles>")
    parser.add_option('', '--no-preselection', dest='no_preselection', default=False, action="store_true",
                      help="No Preselection output when input stream only contains single presentation")
    ...
    parser.add_option('', '--have-hdr', dest="have_HDR", action='store_true', default=False,
                      help="Indicates High Dynamic Range content existence for input source")
    parser.add_option('', '--have-hfr', dest="have_HFR", action='store_true', default=False,
                      help="Indicates High Frame Rate content (>29.97 fps) existence for input source")
    parser.add_option('', '--have-nga', dest="have_NGA", action='store_true', default=False,
                      help="Indicates AC4 content existence for input source")
    parser.add_option('', '--have-non-nga', dest="have_Non_NGA", action='store_true', default=False,
                      help="Indicates audio tracks other than AC4 existence for input source")
    ...
    parser.add_option('', "--attributes", dest="attributes", action="append", metavar='<attributes-definition>', default=[],
                      help="Specify the attributes of a set of tracks. This option may be used multiple times, once per attribute set. Refering to https://confluence.dolby.net/kb/display/CET/ONDEL+Bento4 for example usages.")
    parser.add_option('', '--triveni', dest="triveni", action="store_true",
                      help="Don't create the sub-folder for media content, and only work in " + ISOFF_BROADCAST_PROFILE + "or" + HBBTV_15_ISOFF_LIVE_PROFILE + " profile", default=False)
    parser.add_option('', '--all-input-signals', dest="all_input_signals", action="store_true",
                      help="Process all the input signals", default=False)
    ...
    parser.add_option('', "--playready-license-uri", dest="playready_license_uri",
                      help="Specify the license/key URI to use for PlayReady")
    ...
    parser.add_option('', "--widevine-license-uri", dest="widevine_license_uri",
                      help="Specify the license/key URI to use for Widevine")
    ...
    parser.add_option('', '--segment-profiles', dest="segment_profiles", action='store_true', default=False,
                      help="Add segmentProfiles to Representation for CMAF compliance, use together with --dv-new-signaling")
    parser.add_option('', '--dv-use-supplemental', dest="dv_use_supplemental", action='store_true', default=False,
                      help="Use SupplementalProperty to signal Dolby Vision back compaitable track for DASH.")
    parser.add_option('', '--ajoc-dolby-channel-config', dest="dolby_channel_config", default=False, action='store_true',
                        help="Use Dolby channel configuration for audio tracks (default is to use MPEG channel configuration) only applies to AJOC audio tracks")
    ...
    if not options.profiles:
        ...
        for profile in options.profiles.split(','):
            ...
            if profile in ProfileAliases:
                ...
                raise Exception('profile(s) must be set correctly. (live, on-demand, broadcast, hbbtv)')
    ...
    if HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
        ...
        if options.playready_add_pssh:
            sys.stderr.write('INFO: since hbbtv profile is selected, no PlayReady PSSH box will be added to the init segments\n')
    ...
    # --triveni parameter sanity check 
    if options.triveni == True and ISOFF_BROADCAST_PROFILE not in options.profiles and HBBTV_15_ISOFF_LIVE_PROFILE not in options.profiles:
        raise Exception('--triveni parameter only works in broadcast profile')
    
    if ISOFF_BROADCAST_PROFILE in options.profiles and len(options.profiles) != 1:
        raise Exception('broadcast profile cannot work with other profiles, please use --profiles broadcast')
        
        
    if HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles and len(options.profiles) != 1:
        raise Exception('hbbtv profile cannot work with other profiles, please use --profiles hbbtv')
        ...
        if len(tracks) > 1:
            ...
            for track in tracks[1:]:
                if track.segment_scaled_durations[:-1] != anchor.segment_scaled_durations[:-1]:
                    sys.stderr.write('WARNING: video tracks are not aligned "'+str(track)+'" and "'+str(prev_track)+'"\n')
    ...
    for adaptation_sets in [audio_sets, video_sets, subtitles_sets]:
        for adaptation_set_name, tracks in list(adaptation_sets.items()):
            for track in tracks:
                if ISOFF_BROADCAST_PROFILE in options.profiles or HBBTV_15_ISOFF_LIVE_PROFILE in options.profiles:
                    track.representation_id = str(representation_id)
                    representation_id += 1
                    if options.split:
                        track.init_segment_name = SPLIT_INIT_SEGMENT_NAME
                    else:
                        PrintErrorAndExit('ERROR: For %s or %s, the media content must be segmented' % ISOFF_BROADCAST_PROFILE, HBBTV_15_ISOFF_LIVE_PROFILE)
                else:
            ...
            for adaptation_sets in [audio_sets, video_sets, subtitles_sets]:
                for adaptation_set_name, tracks in list(adaptation_sets.items()):
                    ...
                    for track in tracks:
                        ...
                        if options.triveni == True:
                            final_init_segment_name  = path.join(out_dir, track.representation_id + '_' + track.init_segment_name)
                            final_media_segment_name = path.join(out_dir, track.representation_id + '_' + SEGMENT_PATTERN)
                        else:
                            final_init_segment_name  = path.join(out_dir, track.init_segment_name)
                            final_media_segment_name = path.join(out_dir, SEGMENT_PATTERN)
                        Mp4Split(options,
                                 track.parent.media_source.filename,
                                 track_id               = str(track.id),
                                 pattern_parameters     = 'N',
                                 start_number           = '1',
                                 init_segment           = final_init_segment_name,
                                 media_segment          = final_media_segment_name)
        else:
    ...
    for video_tracks in video_sets.values():
        if ContainDolbyVision(video_tracks):
            options.have_HDR = True
    # # Dual entry for Dolby Vision profile 8 and 9
    # DolbyVisionDualEntry(video_sets, 'hvc1', 'dvh1')
    # DolbyVisionDualEntry(video_sets, 'hev1', 'dvhe')

    # DolbyVisionDualEntry(video_sets, 'avc1', 'dva1')
    # DolbyVisionDualEntry(video_sets, 'avc3', 'dvav')
